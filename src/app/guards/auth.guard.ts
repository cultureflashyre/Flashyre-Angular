...
export const authGuard: CanActivateFn = (route, state) => {
  ...
  // 1. Check if the user is logged in (access token exists in localStorage).
  // We allow access tokens through even if expired because the HTTP interceptor will
  // seamlessly refresh the token via the HttpOnly cookie on the first API call the route makes.
  if (!token) {
